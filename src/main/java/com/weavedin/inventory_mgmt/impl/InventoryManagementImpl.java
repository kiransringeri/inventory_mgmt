...
public class InventoryManagementImpl implements InventoryManagementAPI {
  ...
  private Collection<String> updateItemData(Item itemInDB, Item updatedItem) {
  ...
  private Collection<String> updateVariantData(Variant variantInDB, Variant updatedVariant) {
    List<String> updatedProps = new ArrayList<>();
    ...
    Map<String, String> otherPropsInDB =
        variantInDB.getProperties() == null ? new HashMap<>() : variantInDB.getProperties();
    Map<String, String> otherPropsUpdated = updatedVariant.getProperties() == null ? new HashMap<>()
        : new HashMap<>(updatedVariant.getProperties());
  ...
  private String getItemName(SessionFactory sessionFactry, long itemId) {
    Query<String> query =
        sessionFactry.getCurrentSession().createSQLQuery("select name from item where id=:itemId");
